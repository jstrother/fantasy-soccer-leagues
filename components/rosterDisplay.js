...
import {
	...
	removeStarter,
	removeBench
} from '../flow/subActions/fantasyClubActions.js';
...
export class Display extends React.Component {
	handleRosterRemove(event) {
		...
		if (player.position === 'G' || player.position === 'Goalkeeper') {
			this.props.dispatch(
				removeGoalkeeper(this.props.accessToken, this.props.userId, player)
			);
		}
		if (player.position === 'D' || player.position === 'Defender') {
			this.props.dispatch(
				removeDefender(this.props.accessToken, this.props.userId, player)
			);
		}
		if (player.position === 'M' || player.position === 'Midfielder') {
			this.props.dispatch(
				removeMidfielder(this.props.accessToken, this.props.userId, player)
			);
		}
		if (player.position === 'F' || player.position === 'Attacker') {
			this.props.dispatch(
				removeForward(this.props.accessToken, this.props.userId, player)
			);
		}
		this.props.starters.filter(p => {
			...
			if (sListId === player.idFromAPI) {
				this.props.dispatch(
					removeStarter(this.props.accessToken, this.props.userId, p)
				);
			}
		});
		this.props.benchwarmers.filter(p => {
			...
			if (bListId === player.idFromAPI) {
				this.props.dispatch(
					removeBench(this.props.accessToken, this.props.userId, p)
				);
			}
		});
	}
	...
	addStartingPlayer(event) {
		...
		if (this.props.starters && this.props.starters.length < 11) {
			...
			if (benchPlayerCheck.length === 0) {
				this.props.dispatch(
					addStarter(this.props.accessToken, this.props.userId, player)
				);
			}
		} else {
		...
		if (this.props.benchwarmers && this.props.benchwarmers.length < 7) {
			...
			let starterCheck = this.props.starters.filter(starter => {
				if (player.idFromAPI === starter.idFromAPI) {
					this.props.dispatch(
						warning('This player is already in your starting eleven.')
					);
					return true;
				}
			});
			...
			if (starterCheck.length === 0) {
				this.props.dispatch(
					addBench(this.props.accessToken, this.props.userId, player)
				);
			}
		} else {
	...
	render() {
		...
		return (
			<div className={styles.rosterDisplay}>
				<div className={styles.completeRoster}>
					...
					<table className={styles.rosterTable}>
						...
						<tbody>
							{roster.map(p => {
								return (
									<tr id={`ros-${p.idFromAPI}`} key={`key-${p.idFromAPI}`}>
										<td
				...
				<div
					className={
						this.props.starters.length + this.props.benchwarmers.length === 0
							? styles.hidden
							: styles.lineup
					}>
					<h4 className={styles.lineupHeader}>
						Set your lineup for upcoming matches.
					</h4>
					<div className={styles.lineupHolder}>
						<Warning />
						<div className={styles.startingEleven}>
							<h4 className={styles.startersHeader}>Your 11 Starters:</h4>
							<StartingEleven />
						</div>
						<div className={styles.benchPlayers}>
							<h4 className={styles.benchHeader}>Your 7 Reserves:</h4>
							<BenchPlayers />
						</div>
					</div>
				</div>
...
const RosterDisplay = connect(mapDisplayStateToProps)(Display);
